...
gui.onChange(() => render(device))
{
  ...
  f.add(settings.translation, 2, -1400, 1000, 10).name('z')
}
{
...
const zToMatrix = new Float32Array(16)

function makeZToMatrix(fudgeFactor: number) {
  // prettier-ignore
  zToMatrix.set([
    1, 0, 0, 0,
    0, 1, 0, 0,
...
function render(device: GPUDevice) {
  ...
  // uniformの更新
  const projection = mat4.ortho(0, canvas.clientWidth, canvas.clientHeight, 0, 1200, -1000)
  mat4.multiply(makeZToMatrix(settings.fudgeFactor), projection, matrixValue)

import { clamp, degToRad } from '@/modules/common/math'
...
const uniformBufferSize = (9 + 3 + 16 + 16 + 4 + 3 + 1 + 3 + 1 + 3 + 1 + 1 + 3) * 4
...
const lightWorldPositionValue = uniformValues.subarray(kLightWorldPositionOffset, kLightWorldPositionOffset + 3)
const viewWorldPositionValue = uniformValues.subarray(kViewWorldPositionOffset, kViewWorldPositionOffset + 3)
const shininessValue = uniformValues.subarray(kShininessOffset, kShininessOffset + 1)
const lightDirectionValue = uniformValues.subarray(kLightDirectionOffset, kLightDirectionOffset + 3)
const innerLimitValue = uniformValues.subarray(kInnerLimitOffset, kInnerLimitOffset + 1)
const outerLimitValue = uniformValues.subarray(kOuterLimitOffset, kOuterLimitOffset + 1)
...
gui
  .add(settings, 'innerLimit', 0, 90, 1)
  .onChange((v: number) => {
    if (v >= settings.outerLimit) {
      settings.outerLimit = clamp(0, 90, v + 1)
    }
  })
  .listen()
gui
  .add(settings, 'outerLimit', 0, 90, 1)
  .onChange((v: number) => {
    if (v <= settings.innerLimit) {
      settings.innerLimit = clamp(0, 90, v - 1)
    }
  })
  .listen()
gui.add(settings, 'aimOffsetX', -50, 50, 1)
gui.add(settings, 'aimOffsetY', -50, 50, 1)
...
function render() {
  ...
  innerLimitValue[0] = Math.cos(degToRad(settings.innerLimit))
  outerLimitValue[0] = Math.cos(degToRad(settings.outerLimit))

  const mat = mat4.aim(lightWorldPositionValue, [target[0] + settings.aimOffsetX, target[1] + settings.aimOffsetY, 0], up)
